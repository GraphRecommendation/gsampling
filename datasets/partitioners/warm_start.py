...
from datasets.partitioners.utilities import get_ratings_matrix, set_seeds
...
def _test(unique, indices, counts, seen_indices, seen_counts, rating_matrix: np.ndarray, seen_ratings: np.ndarray):
    ...
    has_time = rating_matrix.shape[1] == 3
    ...
    for i in tqdm(np.arange(len(unique))):
        ...
        users[i] = LeaveOneOutUser(
            user,
            list(map(tuple, seen_ratings[seen_index:seen_index + seen_count, [0, 1]].tolist())),
            list(map(tuple, rating_matrix[index:index + count, [0, 1]].tolist())),
            list(map(tuple, seen_ratings[seen_index:seen_index + seen_count, [0, 2]].tolist())) if has_time else None
        )
