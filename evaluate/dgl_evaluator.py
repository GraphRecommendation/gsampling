...
parser.add_argument('--require_state', action='store_true', help='Require state to be present for all models.')
...
def evaluate_fold(out_path: str, model_names, fold: Fold, feature_configurations: List[FeatureConfiguration], seed: int,
                  ...
                  parameter_value=None, other_model=None, model_value=None, require_state=False):
    """
    Evaluate models
    ...
    for name in model_names:
        ...
        if state is not None:
            ...
            logger.warning('State is required but not found. Skipping model.')
            continue
...
def run():
    ...
    data_path, out_path, experiments, models, test_batch, seed, folds, presample, workers, \
        other_state, other_parameters, parameter_value, other_model, model_values, feature_names, require_state\
        = args.values()
            ...
            for all_fold in iterator:
                ...
                if folds is None or fold.name in folds:
                    ...
                    evaluate_fold(out_path, models, fold, feature_configurations, seed, workers, presample, test_batch,
                                  other_fold, other_params, parameter_value, other_model, model_values, require_state)
                else:
